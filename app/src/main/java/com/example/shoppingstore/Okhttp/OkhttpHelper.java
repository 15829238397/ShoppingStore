...
import com.example.shoppingstore.Bean.GoodsInformationSliderData;
import com.example.shoppingstore.application.MyApplication;
import com.example.shoppingstore.myException.GET_RESPONSE_MESSAGE_FAILURE;
import com.example.shoppingstore.myException.GSON_ANALYZE_MESSAGE_FAILURE;
...
public class OkhttpHelper {
    ...
    private Gson gson ;
    private List<GoodsInformationSliderData> goodsInformationSliderDatas;
    ...
    public void doGet (String uri , BaseCallback callback ){
        Request request = buildRequest(uri , METHOD_TYPE.GET , null ) ;
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Response response) throws IOException {
                ...
                if(response.isSuccessful()){
                    ...
                    callbackError(callback , response , null);
//                    callback.onErroe(response , response.code() , new GET_RESPONSE_MESSAGE_FAILURE("获取服务器信息失败"));
                }
//
//                String json = response.body().string() ;
//                Type type = new TypeToken<List<GoodsInformationSliderData>>(){}.getType() ;
//                goodsInformationSliderDatas = gson.fromJson(json ,type ) ;
//                Log.d("--------------" + goodsInformationSliderDatas.get(0).getTitle(), "" +goodsInformationSliderDatas.get(0).getGoodsImage() ) ;
            }
        });
    }
